...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \REPEAT 
			\STATE Find the index $k$ of the first extended packet $\widehat{p}_k$ for which the \emph{flow selection function} $\varphi((\widehat{p}_n)_{n\in J_{i-1}},\, \widehat{p}_{\alpha}) = true$
...
Let $(p_i)_{i \in S}, (\widehat{p}_i)_{i \in S}, S \subseteq \mathbb{N}$ be (possibly finite) corresponding sequences of packets and extended packets respectively, $\varphi$ a \emph{flow selection function}.
...
We can see that the flow creation process significantly depends upon the implementation of the \emph{flow selection function}. We will discuss the common implementations in the Subsection~\ref{subsec:flow-aggregation}.
...
\subsection{Flow Aggregation}\label{subsec:flow-aggregation}
