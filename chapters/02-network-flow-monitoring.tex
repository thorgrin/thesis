...
\begin{definition}\label{def:flow}
    ...
    A \emph{flow} is defined as a set of packets passing an \emph{observation point}
...
\begin{align*}
	\mathcal{P} = (p_i) i \in \mathcal{N}, p_i \in P\\
	\mathcal{Q} = (q_i) i \in \mathcal{N}, q_i \in Q
\end{align*}
denote sequence of packets and extended packets respectively.
...
A flow $\mathcal{F}$ is a subsequence of packets defined by a sequence of extended packets $\mathcal{Q}$ and a flow selection function $\varphi$. We require that each extended packet $q \in \mathcal{Q}$ should belong to at most one flow. Therefore, we define the flow by induction as follows:
\begin{enumerate}
	\item Denote $\mathcal{Q}_0 = \mathcal{Q}$\improve{rewrite as algorithm}
	\item Apply flow selection function $\varphi$ to extended packets in $\mathcal{Q}_0$
	\item Matching packets form the flow $F_1$
	\item Remove matched extended packets from $\mathcal{Q}_0$, denote the new sequence $\mathcal{Q}_1$
	\item Apply flow selection function $\varphi$ to extended packets in $\mathcal{Q}_1$
	\item \ldots
\end{enumerate}

The following flow definition reflects the inductive nature of flow creation and also provides a guide for constructing flows in real world.

\begin{equation}
\mathcal{F}_t(i) = P_{t_i}, where ...
\end{equation}
