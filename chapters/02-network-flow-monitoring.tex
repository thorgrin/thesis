...
\begin{enumerate}[noitemsep]
	\item Denote $\widehat{\mathcal{P}}_1 = \widehat{\mathcal{P}}$\improve{rewrite as algorithm}
...
The following flow definition reflects the inductive nature of flow creation and also provides a guide for constructing the flows. We define the extended flow $\mathcal{F}_k$ as a subsequence of $\widehat{\mathcal{P}}_{k_i}$:
\begin{equation}
\widehat{\mathcal{F}}_k = (\widehat{p}_{k_i})_{i \in \mathbb{S}_k}, \mbox{ where } \widehat{p}_{k_i} \in \widehat{\mathcal{P}}_k
\end{equation}
Let $\widehat{\mathcal{F}}_k(i)$ be defined inductively for $i$:
\begin{align}
\begin{split}
\bullet\quad i &= 1: 
	\begin{array}{l}
...
\bullet\quad i &> 1: 
	\begin{array}{l}
		\mbox{ if } \left\{ j \in \mathbb{N} | j > k_{i-1} \land \varphi(\widehat{p}_{k_1}, \widehat{p}_{k_2}, \ldots, \widehat{p}_{i-1}, \widehat{p}_j) = true \right\} = \emptyset \mbox{ then } \widehat{\mathcal{F}}_k = (\widehat{p}_{k_j})_{j=1}^{i-i} \\
		\mbox{ else } \widehat{\mathcal{F}}_k(i) = \widehat{p}_{k_i}, \mbox{ where } k_i = \mbox{ min }\left\{ j \in \mathbb{N} | j > k_{i-1} \land \varphi(\widehat{p}_{k_1}, \widehat{p}_{k_2}, \ldots, \widehat{p}_{i-1}, \widehat{p}_j) = true \right\}
	\end{array}
\end{split}
\end{align}
...
Note that for for finite extended flows it holds that $\mathbb{S}_k = \{1, \ldots, n\}$ where $n \in \mathbb{N}$ is the largest number for which $F_k(n)$ is defined. If a flow is infinite than its size is $\infty$ and $\mathbb{S}_k = \mathbb{N}$.
