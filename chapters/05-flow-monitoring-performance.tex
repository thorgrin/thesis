...
% supported by almost all modern cards, but needs to be used carefully
...
\itodo{We were the first to implement and measure, more in the next section\\
the best way would be to preformat the data so that it could be used by the flow exporter directly without costly memory movement operations}
...
\itodo{Describe that entire flow monitoring was proposed in NIC \cite{Zadnik-2008-Network}}
\itodo{describe why it can lead to incorrect results when rules are applied with delay}
...
\itodo{referencovat \cite{Gallenmueller-2015-Comparison}, je potreba zmerit, co se nam jeste vyplati delat}
...
\itodo{klidne pouzit dve ruzne karty pro ruzne smery}
