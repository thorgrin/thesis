...
% 2010 PF_RING with RSS reature, numa aware, DNA eventually became ZC (packet fanout across multiple threads, cloud ready) and is available today as such.
When commodity NICs started to support Receive Side Scaling (RSS), the packet capture frameworks started supporting this feature to better utilize multi-core CPUs. \citeauthor{Fusco-2010-High} showed how RSS can be utilized by multi-threaded polling together the the zero copy approach~\cite{Fusco-2010-High} in~\citeyear{Fusco-2010-High}. The basic idea was to use one kernel polling thread for each receive (RX) queue and make the associated buffer available to one user-space capture thread. This approach was used together with the PF_RING framework and used custom driver for zero copy packet reception. The authors show that it is important to correctly configure the system to work on NUMA architecture by setting CPU affinity of the individual polling and packet capture threads. Therefore, this approach mitigates the limitations of Linux NAPI discussed in the previous section. % TODO mention pf_ring zc and discuss difference between handling the buffers in kernel (pf_ring) and in userspace (dpdk).

% 2012 netmap, dpdk, pfq

% basically all FPGA based cards have some kind of zero copy driver (Napatech, Myricom, Mellanox Technologies, or Netcope Technologie, Endace), new Intel cards are often matching the capablities
% lots of drivers that try to limit copying and bypass the kernel (UIO drivers)

% TODO list performance comparison papers? implementation details are important, each framework has specific aim and users and thus features.
...
% netmap 2012
% PFQ 2012
% DPDK 2012 (http://dpdk.org/doc/nics)
% PF_RING 2005
