...
% Some concepts are novel, some can already be found in the literature
...
\itodo{%
abolish or update the table \ref{tab:flow-acc-techniques}, it needs to point to the next section as well. It should probably contain only approaches for this section, maybe show as usable for basic/application flow monitoring\\
make clear which proposals are novel (my own), ticks/emph, maybe two table side by side
}
\begin{table}[ht!]
    \centering
    ...
    Receive Side Scaling && Multithreading & \\ \hline
    Packet trimming && NUMA awareness & \\ \hline
    Packet header preprocessing && Flow state in parsers & \cmark \\ \hline
    Flow processing offloading && Flow cache design & \\ \hline
    Application identification & \cmark & Flow expiration & \\ \bottomrule
    \end{tabular}
...
% TODO use subsection or even subsubsections for different methods to increase readability
