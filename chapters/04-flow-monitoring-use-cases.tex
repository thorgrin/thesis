...
% \emph{(i)} we provide an enhanced version of flow-based IPv6 measurement prototype, which enables IPv6 visibility in large-scale networks, \emph{(i)} we provide a flow-based IPv6 measurement system prototype, which enables IPv6 visibility in large-scale networks,
...
%Flexible IPFIX collector is needed to handle the flow records containing information about IPv6 tunnels. We use IPFIXcol~\cite{ipfixcol} framework to receive and store the extended flow data, since new elements can be defined in configuration XML and used without any further difficulties. FastBit database was chosen as a data storage for the flow records. The data from observation points are sent to the collector over TCP protocol, to ensure that no flow records are lost during transport.
...
\subsection{Frequency of TTL and HOP Values}
...
\section{IPv6 Tunneled Traffic Analysis} \label{subsec:ipv6-tunnels-inner-traffic-evaluation}
...
\begin{table}
    \centering % TODO try to put side by side in a smaller font
    \subfloat[Based on Teredo IPv6 addresses]{
        \small
        \begin{tabular}{rccc}
    ...
    \vspace{3mm}
    \subfloat[Based on UDP port 3544]{
        \small
        \begin{tabular}{rccc}
            \textbf{Server IP} & \textbf{Ratio} & \textbf{Owner} & \textbf{Country} \\ \toprule 
...
\subsection{Location of Tunnel Endpoints}
