...
\thesissetup{
    ...
    keywords      = {network, monitoring, measurement, flow, application flow, NetFlow, IPFIX, performance, 100Gbps},
    TeXkeywords   = {network, monitoring, measurement, flow, application flow, NetFlow, IPFIX, performance, 100\,Gbps},
...
To keep pace with changes in network operation and innovative network attacks network flow monitoring is constantly evolving. This thesis contributes to the continuous efforts by exploring the possibilities unlocked by extending the flow data with application-specific information. We show how the construction of flows is affected by the addition, present the benefits to traffic analysis and assess the inevitable performance loss. To compensate for the lost performance, several novel optimization techniques are proposed for the flow monitoring process. Recognizing that the increasing deployment of encryption is going to limit the benefits of application flow monitoring, we perform a survey of methods for measurement of encrypted traffic. The thesis is concluded by an outlook towards future possibilities for flow monitoring advancement.
...
% TODO we can split literature by type, keywords, etc.
% http://ctan.mirrors.hoobly.com/macros/latex/contrib/biblatex/doc/biblatex.pdf#subsubsection.3.13.4
% \printbibheading
% \printbibliography[nottype=onlineheading=subbibliography,title={Printed Sources}]
