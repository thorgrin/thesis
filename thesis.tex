...
%% motivace
% proc je potreba sledovat sit 
\noindent The Internet has become a crucial medium for business, entertainment, communication, learning, access to information, and other human endeavours. As the society becomes increasingly dependent upon the availability and security of online services, the revenue from disrupting the normal operation of these service increases as well. Gaining unauthorized access, disrupting functionality, stealing and selling confidential data, and impersonation are only a few examples of these disruptions. Various methods, such as firewalls and anti-virus software, are intended to thwart such attacks and reduce the associated risks. To protect users on their networks, network administrators often deploy network monitoring systems to detect and mitigate the threats.
% The ubiquity of network attacks has become a status quo in network

% pro velky site je dobre pouzivat flow, da se slusne analyzovat a narozdil od DPI skaluje
% utoky jsou cim dal slozitejsi, musi se rozvijet i monitorovani site - aplikacni flow
% ruzne informace z aplikacnich vrstev se do flow pridavaji
Network flow monitoring is an instrument which allows observing traffic at some point in the network and exporting aggregated information about the observed network connections. The aggregated nature of provided information allows to scale the flow monitoring to high-speed networks and monitor traffic rates up to and including hundreds of gigabits per second. Although the main utilization of flow data is for security purposes, it is a versatile so that is often used for network management task, such as capacity planning and accounting, as well. As both network communication protocols and attack vector are becoming increasingly complex, the flow monitoring needs to evolve as well. The current trends for flow monitoring are extraction of supplementary information from application protocols, identification of encrypted traffic, and monitoring of high-speed networks.

% contributions
The topic of this thesis is flow monitoring with special focus on application-aware flow monitoring. Our first contribution is a revised definition of flow that attempts to improve and update currently used definitions so that it better matches current flow monitoring practices. Formal definition of flow, together with algorithms for flow construction based on this definition is provided as well. We demonstrate that the revised flow definition allows for use cases that were not covered by NetFlow v9 and IPFIX definitions, such as a monitoring of traffic with fragmented packets.

Using the revised terminology and definitions, we focus on application flow monitoring, which is one of the current trends in flow monitoring. Firstly, an overview of the state of application flow monitoring is provided. Secondly, a workable definition of application flow and application flow record are given to explicitly state what is considered to be application flow monitoring and what is not. Thirdly, an experimental study of an HTTP application parser design is presented. The study illustrates how application flow monitoring increases the demand on computational resources and decreases the performance of the flow monitoring system.

The application flow monitoring provides new data for traffic analysis. We study several use cases for which the application flow monitoring can be applied in detail. The first one uses information from HTTP headers to detect new classes of attacks on application layer. The second one shows how the additional information can be used to analyse utilisation of IPv6 transition mechanisms. Then, we show that adding geolocation information to flow records can be used for advanced traffic analysis. And last, a method for characterising network traffic is described. It makes it possible to compare multiple network traces and search for similarities.

The monitoring of high-speed networks is another of the current trends in flow monitoring. Firstly, we describe the state-of-the art in this area. Secondly, we identify and propose multiple optimisation including those that rely on programmable network interface cards. Some of these optimisations are then used to build and demonstrate a high-density flow monitoring system capable of processing sixteen 10\,Gb link in a single box.
...
\usepackage[paper=a4paper,top=2.5cm,bottom=2.5cm,left=2.5cm,right=2.5cm,foot=1cm]{geometry}

% Chapter titles
\usepackage[noindentafter]{titlesec}
% \titleformat{\chapter}[display]
% {\raggedleft}
% {\fontsize{40}{30}\selectfont\thechapter\vspace{0.5em}}
% {0pt}
